...
----
📋 Resumen de Errores Comunes en Migración React → Astro + Svelte 5
🔧 Errores de Configuración
1. PostCSS/Tailwind Integration
ERROR: [postcss] configDependencies is not iterable
ERROR: The `border-border` class does not exist
Causa: Configuración incorrecta de Tailwind con Astro Solución:

Usar tailwindcss@3.4.17 (no v4)
Configurar postcss.config.cjs manualmente
Definir colores personalizados en tailwind.config.js
2. Dependencias Faltantes
ERROR: Cannot find package '@astrojs/svelte'
ERROR: Cannot find module 'lucide-svelte'
Solución: Instalar dependencias específicas

bash
pnpm add @astrojs/svelte lucide-svelte
🎯 Errores de Sintaxis Svelte
3. Etiquetas Autocerradas
ERROR: Self-closing HTML tags for non-void elements are ambiguous
Problema: <div /> en Svelte no es válido Solución: Usar <div></div> para elementos no-void

4. Imports Incorrectos
ERROR: (0 , __vite_ssr_import_2__.useState) is not a function
Problema: useState no existe en Svelte Solución: Usar variables simples
...
ERROR: Cannot destructure property 'children' of 'props' as it is undefined
Problema: Sintaxis React en componentes Svelte Solución:

javascript
// ❌ Incorrecto
return new Monitor({ size: 16 });
 
// ✅ Correcto
return Monitor;
// <svelte:component this={getIcon(cat)} size={16} />
🎨 Errores de Estilos
6. Clases Tailwind No Definidas
ERROR: The `outline-ring/50` class does not exist
Solución: Definir colores en tailwind.config.js
...
7. Estructura HTML Incorrecta
Problema: Divs vacíos o mal anidados Solución: Mantener exactamente la misma estructura HTML del original
...
🚀 Reglas de Oro
Sin etiquetas autocerradas en elementos no-void
Variables simples en lugar de hooks React
Componentes dinámicos con svelte:component
Misma estructura HTML que el original
Definir colores personalizados en Tailwind
Verificar dependencias específicas de Svelte
Este resumen cubre los errores más comunes y sus soluciones para futuras migraciones.
